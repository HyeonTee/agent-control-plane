...
func TestHealthDoesNotDependOnDatabase(t *testing.T) {
	...
	NewHandler(testPinger{err: errors.New("down")}, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/health", nil))
...
func TestReadyReportsDatabaseState(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			r := httptest.NewRecorder()
			NewHandler(testPinger{err: tc.err}, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/ready", nil))
			if r.Code != tc.want {
